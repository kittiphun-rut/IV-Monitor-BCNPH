...
bool anyBedDripping() {
  if (currentOledPage == 0) {
    for (int i = 0; i < activeStationCount; i++) {
      if (dripModeOf(i) == DRIP_FLOW && dripPeriodMs(i) > 0) return true;
    }
    return false;
  }
...
void renderBlockStyleGrid() {
  ...
  if (activeStationCount == 1) {
    ...
    drawDripChamber(108, topY + 16, 24, 0);     // กระเปาะหยดจำลองของเตียงนี้
    ...
    if (!online) {
      ...
      u8g2.setFont(u8g2_font_5x8_tf);
      u8g2.drawStr(64, topY + 26, "mL/h");
    }

    char subBuf[40];
    snprintf(subBuf, sizeof(subBuf), "Vol %3.0f/%.0fmL  Link %d%%",
             stations[0].totalVolumeMl, stations[0].cfg.planVolumeMl, stations[0].linkPct);
    u8g2.setFont(u8g2_font_4x6_tf);
    u8g2.drawStr(6, topY + 39, subBuf);
  }
  else if (activeStationCount == 2) {
    int cardW = 62;
    for (int i = 0; i < 2; i++) {
      int x = (i == 0) ? 0 : 66;
      // กรอบการ์ด + แถบหัวทึบ (เดิมทึบทั้งใบ ทำให้วาดกระเปาะหยดทับไม่ได้)
      u8g2.drawRFrame(x, topY + 1, cardW, totalH - 1, 2);
      u8g2.drawBox(x + 1, topY + 2, cardW - 2, 10);
      ...
      if (!online) {
        u8g2.setFont(u8g2_font_6x10_tf);
        u8g2.drawStr(x + 3, topY + 26, "OFF");
      } else if (!stations[i].isRunning) {
        u8g2.setFont(u8g2_font_6x10_tf);
        u8g2.drawStr(x + 3, topY + 26, "PAUSE");
      } else {
        char rBuf[16];
        snprintf(rBuf, sizeof(rBuf), "%3.0f", stations[i].flowRate_ml_hr);
        u8g2.setFont(u8g2_font_helvB12_tf);
        u8g2.drawStr(x + 3, topY + 28, rBuf);
        u8g2.setFont(u8g2_font_4x6_tf);
        u8g2.drawStr(x + 29, topY + 28, "mh");
      }

      char vBuf[16];
      snprintf(vBuf, sizeof(vBuf), "%3.0fmL %d%%", stations[i].totalVolumeMl, stations[i].linkPct);
      u8g2.setFont(u8g2_font_4x6_tf);
      u8g2.drawStr(x + 3, topY + 38, vBuf);

      drawDripChamber(x + 44, topY + 15, totalH - 17, i);   // กระเปาะหยดประจำเตียง
    }
  }
  else {
    int cardW = 62;
    ...
    for (int i = 0; i < activeStationCount; i++) {
      ...
      u8g2.drawRFrame(x, y, cardW, rowH - 1, 2);

      // เตียงที่มีเหตุวิกฤตทำป้ายเลขเตียงเป็นแถบทึบ เห็นชัดจากระยะไกลโดยไม่กลบทั้งการ์ด
      char idStr[8];
      snprintf(idStr, sizeof(idStr), "B%d", i + 1);
      u8g2.setFont(u8g2_font_4x6_tf);
      if (critical) {
        u8g2.drawBox(x + 1, y + 1, 10, rowH - 3);
        u8g2.setDrawColor(0);
        u8g2.drawStr(x + 2, textY, idStr);
        u8g2.setDrawColor(1);
      } else {
        u8g2.drawStr(x + 2, textY, idStr);
      }

      drawDripMini(x + 12, y + 1, rowH - 3, i);   // จังหวะหยดของเตียงนี้

      if (!online) {
        u8g2.drawStr(x + 19, textY, "OFFLINE");
      } else if (!stations[i].isRunning) {
        u8g2.drawStr(x + 19, textY, "PAUSED");
      } else {
        char rBuf[24];
        // ชิดขอบพอดี 43 px = 10 อักษร: "100mh 300"  (เดิมเลขติดป้ายเตียงจนอ่านไม่ออก)
        snprintf(rBuf, sizeof(rBuf), "%3.0fmh%4.0f", stations[i].flowRate_ml_hr, stations[i].totalVolumeMl);
        u8g2.drawStr(x + 19, textY, rBuf);
      }
    }
  }
...
void updateHostOLED() {
  ...
  if (oledScreensaverActive) {
    ...
    // กระเปาะหยดอยู่ชิดขวา ข้อความทั้งหมดจึงเหลือความกว้าง 108 px
    drawDripChamber(112, 12, 42, idx);

    char rateBuf[28];
    if (!isOnline) snprintf(rateBuf, sizeof(rateBuf), "Rate: OFFLINE");
    else if (isPaused) snprintf(rateBuf, sizeof(rateBuf), "Rate: 0.0 PAUSE");
    ...
    u8g2.setFont(u8g2_font_5x8_tf);
    char volBuf[32];
    snprintf(volBuf, sizeof(volBuf), "V:%.0f/%.0f Set:%.0f", s.totalVolumeMl, s.cfg.planVolumeMl, s.cfg.targetRateHr);
    u8g2.drawStr(0, 32, volBuf);

    char statusBuf[32];
    if (!isOnline) snprintf(statusBuf, sizeof(statusBuf), "St: NO SIGNAL");
    else if (isPaused) snprintf(statusBuf, sizeof(statusBuf), "St: PAUSED/STOP");
    else if (s.alertCode == ALERT_NEAR_END) snprintf(statusBuf, sizeof(statusBuf), "St: NEXT BAG %s", s.nearEndAck ? "(ACK)" : "!");
    else if (s.alertCode != ALERT_NONE) snprintf(statusBuf, sizeof(statusBuf), "St: !%s!", alertTextEn(s.alertCode));
    else snprintf(statusBuf, sizeof(statusBuf), "St: NORMAL FLOW");
    u8g2.drawStr(0, 42, statusBuf);

    // คุณภาพลิงก์ = ได้รับกี่ % ของแพ็กเก็ตที่ควรได้ใน 10 วินาทีล่าสุด
    // ต่างจาก RSSI ตรงที่บอก "ข้อมูลหายจริงไหม" ไม่ใช่แค่ "สัญญาณแรงแค่ไหน"
    char linkBuf[32];
    if (s.batteryVolts > 0.5)
      snprintf(linkBuf, sizeof(linkBuf), "%ddBm L:%d%% %1.2fV", s.rssi, s.linkPct, s.batteryVolts);
    else
      snprintf(linkBuf, sizeof(linkBuf), "%ddBm Link:%d%%", s.rssi, s.linkPct);
    u8g2.drawStr(0, 52, linkBuf);

    u8g2.drawHLine(0, 55, 128);
    if (isOnline && s.linkPct > 0 && s.linkPct < LINK_WEAK_PCT) {
      u8g2.drawBox(0, 56, 128, 8);
      u8g2.setDrawColor(0);
      u8g2.drawStr(2, 63, "! WEAK LINK - CHECK RF");
      u8g2.setDrawColor(1);
    } else {
