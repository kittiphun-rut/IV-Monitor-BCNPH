/**
 * @file      Station-Stable.ino
 * @brief     เฟิร์มแวร์เครื่องประจำเตียง สายเสถียร ใช้อัลกอริทึมตรวจจับและการคาลิเบรตแบบเดิม
 * @version   7.5.6
 * @date      2026-09-23
 ...
 *
 * @par Description
 * สายคู่ขนานของเครื่องประจำเตียง สำหรับเครื่องที่ยังใช้วิธีคาลิเบรตด้วยมือ 3 ขั้น
 * ต่างจากสายหลักตรงอัลกอริทึมตรวจจับหยดและหน้าคาลิเบรต ส่วนโปรโตคอลและ
 * สูตรคำนวณอัตราการไหลเหมือนกันทุกอย่าง ใช้คู่กับ Host รุ่นเดียวกันได้
 *
 * @par Revision History
 * | Version | Date | Change |
 * |---|---|---|
 * | 7.5.6 | 2026-09-23 | จำนวนหยดสะสมรอดการรีบูตและไฟดับแล้ว เพิ่มสุนัขเฝ้าบ้าน และแก้การนับเวลาที่พังเมื่อ millis() วนรอบ |
 * | 7.5.5 | 2026-09-23 | ย้ายโค้ดวาดจอออกไปไว้ที่ `StationScreen.h` ตรรกะไม่เปลี่ยนแม้แต่บรรทัดเดียว |
 * | 7.5.4 | 2026-09-12 | แก้สูตรอัตราการไหลให้ตรงกับวิธีที่พยาบาลนับหยดจริง |
 * | 7.5.3 | 2026-09-12 | สุ่มจังหวะส่งข้อมูล แก้อาการ Host ขึ้น OFFLINE เมื่อมีหลายเตียง |
 * | 7.5.2 | 2026-09-19 | พอร์ตการหมุนจอ 180 องศาจากสายหลักลงมา |
 * | 7.5.1 | 2026-09-12 | รื้อระบบคัดกรองหยดและหน้าคาลิเบรต 3 ขั้นแบบทำเอง |
...
#include <time.h>
#include <sys/time.h>
#include <esp_task_wdt.h>          // [7.5.6] เพิ่ม: สุนัขเฝ้าบ้าน กันเครื่องค้างเงียบ
#include <esp_system.h>            // [7.5.6] เพิ่ม: อ่านสาเหตุการรีบูตครั้งล่าสุด

// ----------------------------------------------------------------------------
// สุนัขเฝ้าบ้านและสาเหตุการรีบูต ([7.5.6] เพิ่มทั้งหมด)
//
...
// ----------------------------------------------------------------------------
// เก็บจำนวนหยดสะสมให้รอดการรีบูต ([7.5.6] เพิ่มทั้งหมด)
//
...
void applyCounterReset() {
  ...
  totalDrops = 0;
  tallyClear(currentStationId);   // [7.5.6] เพิ่ม: เริ่มถุงใหม่ = ล้างยอดที่เก็บไว้ด้วย
...
void setup() {
  ...
  // [7.5.6] เพิ่ม: กู้จำนวนหยดสะสมของเตียงนี้กลับมา ไฟดับแล้วยอดต้องไม่หาย
  ...
  bootResetReason = esp_reset_reason();   // [7.5.6] เพิ่ม: จำไว้ว่ารีบูตครั้งล่าสุดเพราะอะไร
...
// ----------------------------------------------------------------------------
void loop() {
  feedWatchdog();   // [7.5.6] เพิ่ม: บอกสุนัขเฝ้าบ้านว่ายังเดินอยู่
  ...
  if (lr == 0 || (currentMillis - lr > HOST_TIMEOUT_MS)) {   // [7.5.6] แก้: ตัด currentMillis > lr ทิ้ง เพราะทำให้การวนรอบของ millis() พัง
    isHostOnline = false;
  }
  ...
  totalVolumeMl = (float)totalDrops / (float)safeDropFactor(dropFactor);
  serviceTallySave(currentMillis, totalDrops, currentStationId);   // [7.5.6] เพิ่ม
  ...
  if (currentState == STATE_CONFIG_ID) {
    ...
    if (currentMillis >= configAutoSaveTimeout) {
      if (tempConfigStationId != currentStationId) {
        ...
        stationPrefs.end();
        // [7.5.6] เพิ่ม: ย้ายยอดสะสมไปอยู่ใต้เลขเตียงใหม่ ไม่งั้นบูตหน้าจะกู้ไม่เจอ
