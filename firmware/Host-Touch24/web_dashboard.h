 ...
 * @brief     หน้าเว็บ Dashboard ของเครื่องส่วนกลาง รุ่นจอสัมผัส 2.4 นิ้ว
 * @version   4.10.0
 * @date      2026-09-23
 ...
 *
 * @par Description
 * หน้าเว็บทั้งหน้าเก็บเป็นสตริงเดียวใน PROGMEM แล้วส่งให้เบราว์เซอร์
 *
 * @par Revision History
 * | Version | Date | Change |
 * |---|---|---|
 * | 4.10.0 | 2026-09-23 | หน้าเว็บขึ้นแดงเป็นเหตุวิกฤตเมื่อเตียงที่กำลังให้น้ำเกลือหายไปจากอากาศ |
 * | 4.9.2 | 2026-09-23 | เพิ่มแถบวินิจฉัย คุณภาพลิงก์ และการเตือนเลขเตียงซ้ำ ให้เท่ารุ่นจอ OLED |
 * | 4.9.0 | 2026-09-12 | แยกหน้าเว็บออกจากไฟล์ .ino มาไว้ในไฟล์นี้ |
 *
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Sarabun", sans-serif; }
    ...
    .diag-banner .dg-title { font-weight: 800; color: #f6ad55; margin-bottom: 6px; font-size: 16px; }
    .diag-banner code {
      background: #2d3748; padding: 1px 6px; border-radius: 5px; font-size: 14px;
    }
    .station-grid {
      display: grid;
...
<body>
  ...
  <div class="main-wrap">
    <div id="tab-live" class="tab-content active">
      ...
      <!-- แถบวินิจฉัย: อธิบายกรณีที่หน้าเว็บขึ้น Offline ทั้งที่เครื่องข้างเตียงทำงานปกติ -->
  ...
  <script>
    ...
    function renderStations(stationsData) {
      ...
      stationsData.forEach(st => {
        ...
        // [4.10.0] แก้: เตียงที่หายไประหว่างให้น้ำเกลือ ต้องขึ้นแดงเหมือนเหตุวิกฤตอื่น
        if (!st.online) {
          if (st.alertCode === 7) {
        ...
        card.innerHTML = `
          ...
          <div class="stat-row">
            <span>รหัสเครื่อง (MAC):</span>
            <span class="val" style="color:${st.idConflict ? '#e53e3e' : '#718096'}; font-size:13px;">
              ${st.mac || '-'}${st.idConflict ? ' ⚠️ เลขเตียงซ้ำ' : ''}
            </span>
          </div>
          <div class="stat-row">
            <span>หยดสะสม:</span>
      ...
      // ---- แถบวินิจฉัยปัญหาการเชื่อมต่อ (ใหม่ใน 4.9.2) ----
      const diag = document.getElementById('diagBanner');
      const dupBeds = stationsData.filter(x => x.idConflict).map(x => x.id);
      const beyond = (lastHostData && lastHostData.heardBeyond) || 0;
      const offlineBeds = stationsData.filter(x => !x.online).map(x => x.id);
      let diagHtml = '';
      if (dupBeds.length) {
        diagHtml = `<div class="dg-title">⚠️ มีเครื่องมากกว่าหนึ่งตัวตั้งเลขเตียงเดียวกัน (เตียง ${dupBeds.join(', ')})</div>
          Host แยกเตียงจากเลขที่เครื่องส่งมาเท่านั้น สองเครื่องที่ตั้งเลขซ้ำกันจะเขียนทับช่องเดียวกัน
          เตียงที่เหลือจึงขึ้นว่า Offline ทั้งที่เครื่องทำงานปกติ<br>
          <b>วิธีแก้</b> ที่เครื่องข้างเตียง กดปุ่ม <code>3 ครั้ง</code> เพื่อเข้าหน้า SET BED ID แล้วตั้งเลขให้ไม่ซ้ำกัน`;
      } else if (beyond > 0) {
        diagHtml = `<div class="dg-title">ℹ️ ได้รับข้อมูลจากเตียง ${beyond} แต่ยังไม่ได้เปิดใช้</div>
          ขณะนี้เปิดใช้อยู่ ${activeBedCount} เตียง ข้อมูลของเตียง ${beyond} จึงถูกมองข้าม<br>
          <b>วิธีแก้</b> กด "เพิ่มเตียง" ด้านบนจนครบ หรือเปลี่ยนเลขเตียงที่เครื่องให้อยู่ในช่วง 1-${activeBedCount}`;
      } else if (offlineBeds.length) {
        diagHtml = `<div class="dg-title">ℹ️ เตียง ${offlineBeds.join(', ')} ยังไม่ได้รับข้อมูล</div>
          ถ้าหน้าจอของเครื่องข้างเตียงขึ้นว่า ONLINE อยู่ ให้ตรวจสองอย่างนี้ก่อน<br>
          1. <b>เลขเตียงซ้ำกันหรือไม่</b> — กดปุ่มที่เครื่อง 3 ครั้ง ดูเลขในหน้า SET BED ID ของทุกเครื่อง<br>
          2. <b>เลขเตียงเกินจำนวนที่เปิดใช้หรือไม่</b> — ขณะนี้เปิดใช้ ${activeBedCount} เตียง เลขต้องอยู่ในช่วง 1-${activeBedCount}`;
      }
      diag.innerHTML = diagHtml;
      diag.classList.toggle('show', diagHtml !== '');
