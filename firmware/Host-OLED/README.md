# เครื่องส่วนกลาง จอ OLED v4.7.7 — แยกโค้ดวาดจอออกเป็น `HostScreen.h`
...
```
ย้ายออกไป 365 บรรทัด    ไฟล์หลักเหลือ 1957 บรรทัด
```
...
ได้ผลว่าเหมือนกันทุกจุดครบทั้ง **31 ภาพ**
...
# Central Host Gateway v4.7.6-OLED — นำการเชื่อมต่อเราเตอร์กลับมา เพื่อลดภาระซีพียู

ต่อยอดจาก **Host จอ OLED v4.7.5** โครงสร้างแพ็กเก็ต (Protocol v3) เท่าเดิมทุกไบต์
ตรรกะการวัดและการแจ้งเตือนไม่เปลี่ยน ใช้คู่กับ Station รุ่นเดิมได้ทันที

## ปัญหาที่แก้ (ใหม่ใน v4.7.6)

เมื่อพยาบาลเปิดหน้าเว็บพร้อมกันหลายเครื่อง เครื่อง Host ต้องทำหน้าที่เป็น
Access Point ให้ทุกเครื่องเอง ทั้งส่ง beacon จัดการการเข้าร่วมของแต่ละเครื่อง
และกันบัฟเฟอร์ไว้ต่อเครื่อง งานเหล่านี้แย่งเวลาจากการรับสัญญาณ ESP-NOW ของเตียง

## 1. โหมดเครือข่ายใหม่ — เกาะเราเตอร์เป็นหลัก AP เป็นตัวสำรอง

```
ปกติ    Host --STA--> เราเตอร์โรงพยาบาล
        มือถือ/แท็บเล็ต --> เราเตอร์ --> Host        AP ปิด ภาระ AP = 0

สำรอง   ต่อไม่ได้ใน 15 วินาที หรือเราเตอร์หลุดเกิน 20 วินาที
        Host เปิด AP เอง พยาบาลต่อ SSID เดิมได้เหมือนทุกวัน
```

**ไม่ใช่ `WIFI_AP_STA`** ที่เปิดทั้งสองอย่างค้างไว้แบบ v4.6.0 เพราะแบบนั้น AP
ยังทำงานอยู่ ภาระไม่ได้ลดลงเลย ที่นี่ต่อเราเตอร์สำเร็จเมื่อใด AP ถูกปิดทันที

ขณะใช้ AP สำรอง จะลองกลับไปเกาะเราเตอร์ทุก 10 นาที **แต่เฉพาะตอนไม่มีใครต่อ AP อยู่**
เพราะการสลับโหมดจะตัดการเชื่อมต่อของผู้ใช้ที่กำลังดูหน้าเว็บ

## 2. ช่องสัญญาณของ ESP-NOW — ปล่อยให้เป็นของเราเตอร์

เมื่อเกาะเราเตอร์ ช่องสัญญาณถูกเราเตอร์กำหนด ไม่ใช่ช่อง 1 อีกต่อไป
**เฟิร์มแวร์นี้ไม่บังคับช่องกลับ** เพราะ Station มี `manageChannelHunting()`
ที่ไล่ช่อง 1–13 ช่องละ 2.5 วินาทีเมื่อไม่ได้ยิน Host แล้วจำช่องที่เจอลง NVS
อย่างช้าที่สุดใช้เวลา 13 × 2.5 = **32.5 วินาที** ก็กลับมาออนไลน์ครบทุกเตียง

v4.6.0 เคยเรียก `esp_wifi_set_channel(ESPNOW_CHANNEL)` **หลัง** เกาะเราเตอร์สำเร็จ
ซึ่งดึงวิทยุกลับช่อง 1 ทั้งที่เราเตอร์อยู่ช่องอื่น ลิงก์เราเตอร์จึงพังทันที
เป็นบั๊กที่ต้องไม่ทำซ้ำ และมีคอมเมนต์กำกับไว้ในโค้ดแล้ว

## 3. ปิดโหมดประหยัดพลังงานของวิทยุซ้ำสามจุด

Arduino core เรียก `esp_wifi_set_ps(WiFi.getSleep())` **ทับค่าที่เราตั้งไว้**
ตอนเกิดเหตุการณ์ `ARDUINO_EVENT_WIFI_STA_START` และค่าปริยายของ ESP32-S3 คือ
`WIFI_PS_MIN_MODEM` ผลคือตัวรับหลับเป็นช่วง ๆ แพ็กเก็ตจากเตียงมาถึงช้าเป็นวินาที

เฟิร์มแวร์นี้จึงเรียก `forceRadioAwake()` สามจุดรอบการเข้าโหมด STA —
ก่อน `WiFi.begin()` หลัง `WiFi.begin()` และอีกครั้งเมื่อเชื่อมต่อสำเร็จ

## 4. การค้นหาเครือข่ายแบบไม่บล็อก

`WiFi.scanNetworks()` แบบเดิมบล็อกราว 2 วินาที ระหว่างนั้นวิทยุกระโดดข้ามช่อง
แพ็กเก็ต ESP-NOW หายหมด รุ่นนี้ใช้ `WiFi.scanNetworks(true)` แล้วให้หน้าเว็บ
ถามผลซ้ำทุก 0.9 วินาทีจนกว่าจะเสร็จ

ขณะอยู่โหมด AP ล้วนจะสแกนไม่ได้ เฟิร์มแวร์จึงยืมอินเทอร์เฟซ STA มาชั่วคราว
(`WIFI_AP_STA`) เฉพาะช่วงสแกน แล้วคืนโหมดเดิมพร้อมผูก peer ของ ESP-NOW กลับให้ถูกต้อง

## 5. หน้าเว็บปรับจังหวะรีเฟรชตามจำนวนผู้ใช้

ของเดิมหน้าเว็บเรียก `/api/data` **ทุก 1 วินาทีต่อหนึ่งแท็บที่เปิดอยู่**
ถ้ามี 8 เครื่องเปิดพร้อมกันคือ 8 ครั้งต่อวินาที ผ่าน `WebServer` ที่ทำงานแบบบล็อก
**นี่คือภาระตัวจริง ซึ่งการย้ายไปโหมด STA ไม่ได้ลดเลย**

Host จึงนับอัตราการเรียกในหน้าต่าง 5 วินาที ประเมินจำนวนหน้าเว็บที่เปิดอยู่
แล้วส่ง `pollMs` กลับไปให้หน้าเว็บ ยืดได้ตั้งแต่ 1 ถึง 4 วินาที
และหน้าเว็บเปลี่ยนจาก `setInterval` เป็น `setTimeout` ต่อทอด คำขอจึงไม่ทับถมกันเป็นคิว
เมื่อเครือข่ายช้า (ของเดิมยิงคำขอใหม่ทุก 1 วินาทีไม่ว่าคำขอก่อนหน้าจะเสร็จหรือยัง)

## 6. จอ OLED แสดงที่อยู่ที่ต้องพิมพ์

ที่อยู่หน้าเว็บไม่ใช่ `192.168.4.1` คงที่อีกต่อไป เมื่อเกาะเราเตอร์จะเป็นเลขที่
เราเตอร์แจกให้ จอจึงต้องบอก — แสดงทั้งบน **หน้าพักจอ** (ใต้นาฬิกา) และบน
**หน้า LINK DIAGNOSTIC** โดยคำเตือนเรื่องเลขเตียงซ้ำยังมาก่อนเสมอถ้ามี

## วิธีใช้

1. เปิดหน้าเว็บ กดปุ่ม **จุดเชื่อมต่อ** มุมขวาบน
2. กด **ค้นหา** เลือกเครือข่ายของโรงพยาบาล ใส่รหัสผ่าน
3. กด **บันทึกและเชื่อมต่อ** — หน้านี้จะหลุดเพราะ AP ถูกปิด
4. ต่อมือถือกับ Wi-Fi ของโรงพยาบาล แล้วเปิดที่อยู่ใหม่ที่แสดงบนจอเครื่อง Host
5. ถ้าต้องการกลับไปใช้จุดเชื่อมต่อในตัวถาวร กด **ลืมเครือข่ายนี้**

ถ้าเชื่อมต่อไม่สำเร็จ เครื่องจะเปิด AP กลับมาเองใน 15 วินาที ไม่ต้องทำอะไร

## ข้อจำกัด

เฟิร์มแวร์นี้ **ยังไม่ได้ทดสอบบนฮาร์ดแวร์จริง** ตรวจด้วยเครื่องมือจำลองบน PC เท่านั้น
ชุดตรวจ 9 ตัวใน `tools/` ผ่านครบ และเรนเดอร์หน้าเว็บด้วย Chromium จริงแล้ว
แต่ยังไม่ได้ยืนยันกับเราเตอร์และเตียงจริงว่าการไล่หาช่องใช้เวลาเท่าใดในสนาม
