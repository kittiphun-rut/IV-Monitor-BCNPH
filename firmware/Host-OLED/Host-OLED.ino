 ...
 *
 * @par Hardware
 * ESP32-S3 Dev Module (N16R8) + จอ 1.3" OLED (SH1106 I2C 128x64) + Passive Buzzer
 *
 * @par Description
 * รับข้อมูลจากเครื่องประจำเตียงทุกเตียงผ่าน ESP-NOW ประเมินเหตุที่ต้องแจ้งเตือน
 * แล้วส่งค่าตามแผนการรักษากลับไปให้แต่ละเตียง พร้อมให้บริการหน้าเว็บ Dashboard
 * ไฟล์นี้เก็บตรรกะการรับส่ง การแจ้งเตือน เครือข่าย และ REST API
 * ส่วนที่วาดลงจออยู่ที่ `HostScreen.h` และหน้าเว็บอยู่ที่ `web_dashboard.h`
 *
 ...
 * | 4.7.4-OLED | 2026-09-20 | ตรวจจับเลขเตียงซ้ำ และปรับจอ OLED ให้อ่านง่ายขึ้น |
 * | 4.7.3-OLED | 2026-09-12 | แก้จังหวะส่ง Sync ที่ชนกัน วัดคุณภาพลิงก์ และแอนิเมชันหยด |
 * | 4.7.2-OLED | 2026-09-18 | ปรับหน้าเว็บ Dashboard ตามที่พยาบาลขอ |
 * | 4.7.1-OLED | 2026-09-12 | รองรับ Station หลายสาย และเตือนเมื่อเซนเซอร์ยังไม่จับหยด |
 * | 4.7.0-OLED | 2026-09-16 | ตัดการจัดการ Wi-Fi ออก เหลือโหมด Access Point อย่างเดียว |
 *
...
// [4.7.6-OLED] เพิ่ม: ผูก peer ใหม่ทุกครั้งที่สลับโหมด เพราะอินเทอร์เฟซเปลี่ยนจาก AP เป็น STA
...
// [4.7.6-OLED] เพิ่ม: กันไม่ให้ Arduino core สั่งโหมดประหยัดพลังงานทับตอนเข้าโหมด STA
...
// [4.7.6-OLED] เพิ่ม: AP กลายเป็นโหมดสำรอง ไม่ใช่โหมดหลักเหมือน v4.7.0 ถึง v4.7.5
...
// [4.7.6-OLED] เพิ่ม: เครื่องสถานะของเครือข่าย แทนลูปรอแบบบล็อกของ v4.6.0
...
void handleApiData() {
  ...
  // [4.7.6-OLED] เพิ่ม: ภาระตัวจริงคือหน้าเว็บ poll ทุก 1 วินาทีต่อแท็บ ไม่ใช่โหมด AP
