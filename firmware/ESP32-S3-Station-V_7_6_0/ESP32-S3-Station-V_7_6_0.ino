 ...
 * เวอร์ชัน: 7.6.0 (Protocol v3 — ใช้คู่กับ Central Host Firmware 4.6.0 / 4.7.0 / 4.9.x)
 * บอร์ดประมวลผล: ESP32-S3 Super Mini + จอสี TFT 1.47" (ST7789 SPI 172x320) + Passive Buzzer
 * เซ็นเซอร์หยด: โมดูลเอาต์พุตดิจิทัล (มีวงจรเปรียบเทียบในตัว) ให้พัลส์ Active HIGH ต่อ 1 หยด
 *
 * ---------------------------------------------------------------------------
 * เปลี่ยนใน V7.6.0: เปลี่ยนมาใช้เซ็นเซอร์หยดแบบเอาต์พุตดิจิทัล และจัดผังขาใหม่
 *  - เดิมอ่าน ADC ทุกรอบ loop แล้วเทียบกับ threshold ที่ได้จาก Calibration Wizard
 *    ตอนนี้ใช้ขัดจังหวะ (interrupt) ที่ขอบขาขึ้นของสัญญาณ DO แทน จึงไม่พลาดหยด
 *    แม้ขณะกำลังวาดจอหรือส่ง ESP-NOW และไม่ต้องคาลิเบรตในซอฟต์แวร์อีก
 *    (ความไวปรับที่ตัวโมดูลด้วยโวลุ่ม ดูผลได้จากหน้า SENSOR TEST)
 *  - ผังขาใหม่: DO = GPIO 5, Buzzer = GPIO 6, ปุ่ม = GPIO 7 (เดิม 4 / 3 / 2)
 *    เลิกใช้ GPIO 3 ซึ่งเป็น strapping pin และย้ายทั้งชุดมาอยู่ติดกับบล็อกจอ (8-13)
 *  - ตัด Calibration Wizard, ค่า TH/HYS และ NVS namespace "st_cal" ออกทั้งหมด
 *    กดปุ่มค้าง 3 วินาที = หน้า SENSOR TEST (ดูระดับสัญญาณ จำนวนพัลส์ ความกว้างพัลส์)
 *  - ตรวจตอนบูตว่าขา DO ไม่ได้ค้างอยู่ที่ระดับ HIGH (สายผิดหรือปรับโวลุ่มเลยจุด)
 *  - ตรรกะการคำนวณอัตราไหล การแจ้งเตือน และแพ็กเก็ต Protocol v3 เหมือน V7.5.0 ทุกประการ
 *
 * ---------------------------------------------------------------------------
 ...
 *  6) ลดการวาดจอ TFT ทุกรอบ loop -> วาดเฉพาะเมื่อสถานะเปลี่ยน ไม่แย่งเวลาตรวจจับหยด
...
// ---- เซ็นเซอร์หยดแบบเอาต์พุตดิจิทัล + ปุ่ม + บัซเซอร์ (จัดกลุ่มไว้ติดกับบล็อกจอ 8-13) ----
#define DROP_DO_PIN         5     // ขา DO ของโมดูลเซ็นเซอร์ (พัลส์ 1 ครั้ง = 1 หยด)
#define BUZZER_PIN          6
#define BTN_PIN             7
#define RGB_LED_PIN         48

// ทิศทางสัญญาณของโมดูล: 1 = ปกติ LOW มีหยดแล้วขึ้นเป็น HIGH (Active HIGH)
//                        0 = ปกติ HIGH มีหยดแล้วตกลงเป็น LOW (Active LOW เช่นโมดูล LM393 ส่วนใหญ่)
// เปลี่ยนค่านี้ค่าเดียว โค้ดจะสลับทั้งโหมด pull ภายในและขอบสัญญาณที่ใช้ตรวจจับให้เอง
#define DROP_ACTIVE_HIGH    1

// เวลากันสัญญาณรัว: พัลส์ที่มาเร็วกว่านี้ถือว่าเป็นการสั่นของวงจรเปรียบเทียบ ไม่ใช่หยดใหม่
// 30 ms = รองรับได้ถึงราว 2000 หยด/นาที ซึ่งสูงกว่าการให้สารน้ำจริงมาก
#define DROP_DEBOUNCE_MS    30

// วงจรวัดแบตเตอรี่ (ถ้ามี): ใส่หมายเลขขา ADC ที่ต่อผ่านตัวต้านทานแบ่งแรงดัน 1:1
// GPIO 4 ว่างแล้วหลังย้ายเซ็นเซอร์ จึงใช้เป็นขาวัดแบตเตอรี่ได้
...
// ---- เซ็นเซอร์หยด: ISR เก็บเวลาของแต่ละพัลส์ไว้ในคิว แล้วลูปหลักค่อยมาประมวลผล ----
// ทำแบบนี้เพื่อให้การนับหยดไม่ขึ้นกับว่าลูปหลักกำลังวาดจอหรือส่ง ESP-NOW อยู่หรือไม่
#define DROP_QUEUE_SIZE 8
volatile uint32_t dropQueueUs[DROP_QUEUE_SIZE];
volatile uint8_t  dropQHead      = 0;
volatile uint8_t  dropQTail      = 0;
volatile uint32_t dropIsrCount   = 0;   // จำนวนพัลส์ที่ ISR รับไว้ทั้งหมด (ใช้ในหน้าทดสอบ)
volatile uint32_t dropIsrDropped = 0;   // พัลส์ที่คิวเต็มจนตกหล่น (ปกติต้องเป็น 0)
uint32_t lastDropMicros          = 0;   // เวลาของหยดก่อนหน้า ใช้คำนวณอัตราไหล
bool sensorLineStuck             = false;  // ขา DO ค้างที่ระดับมีหยดตั้งแต่ตอนบูต
...
// ช่วงห่างระหว่างหยดที่สั้นที่สุดที่ยังถือว่าสมเหตุสมผลพอจะนำไปคำนวณอัตราไหล
// (สั้นกว่านี้ = เร็วกว่า 1090 หยด/นาที ซึ่งเป็นไปไม่ได้ในการให้สารน้ำจริง จึงนับหยดแต่ไม่ปรับอัตรา)
...
// ฟังก์ชันช่วย
// ----------------------------------------------------------------------------
// อ่านว่าขณะนี้สาย DO อยู่ที่ระดับ "มีหยดผ่าน" หรือไม่ (ใช้ทั้งหน้าจอและตอนตรวจสายตอนบูต)
inline bool dropLineActive() {
#if DROP_ACTIVE_HIGH
  return digitalRead(DROP_DO_PIN) == HIGH;
#else
  return digitalRead(DROP_DO_PIN) == LOW;
#endif
}
...
void drawHoldProgressHUD(unsigned long holdDur) {
  ...
  if (holdDur < 3000) {
    ...
    drawCenteredString("-> SENSOR TEST (3s)", boxY + 28, 1, COLOR_YELLOW, THEME_NAVY);
    ...
    drawCenteredString("RELEASE = TEST", boxY + 12, 1, COLOR_GREEN, THEME_NAVY);
...
// ----------------------------------------------------------------------------
// หน้าทดสอบเซ็นเซอร์ (กดปุ่มค้าง 3 วินาที) — ใช้ตอนติดตั้งและตอนปรับโวลุ่มบนโมดูล
// เป้าหมาย: หยด 1 หยด ต้องได้พัลส์พอดี 1 พัลส์ ความกว้างราว 20-80 ms
// ----------------------------------------------------------------------------
void executeSensorTestScreen() {
  noTone(BUZZER_PIN);
  buzzerBeepUntil = 0;

  tft.fillScreen(THEME_BG);
  tft.fillRect(0, 0, 172, 28, THEME_PINK);
  drawCenteredString("SENSOR TEST", 8, 1, COLOR_WHITE, THEME_PINK);
  drawCenteredString("TURN THE TRIMMER UNTIL", 38, 1, THEME_SKYBLUE, THEME_BG);
  drawCenteredString("1 DROP = 1 PULSE", 54, 1, COLOR_YELLOW, THEME_BG);

  tft.fillRoundRect(6, 74, 160, 56, 8, THEME_NAVY);
  drawCenteredString("LINE LEVEL", 80, 1, UI_DIM, THEME_NAVY);
  tft.fillRoundRect(6, 138, 160, 56, 8, THEME_NAVY);
  drawCenteredString("PULSES", 144, 1, UI_DIM, THEME_NAVY);
  tft.fillRoundRect(6, 202, 160, 80, 8, THEME_NAVY);
  drawCenteredString("LAST PULSE", 208, 1, UI_DIM, THEME_NAVY);
  drawCenteredString("CLICK = EXIT", 300, 1, COLOR_WHITE, THEME_BG);

  uint32_t baseCount  = dropIsrCount;
  bool     lastLevel  = dropLineActive();
  uint32_t riseUs     = 0;
  uint32_t lastRiseUs = 0;
  float    widthMs    = -1.0f;
  float    gapMs      = -1.0f;

  String cacheLevel = "", cacheCount = "", cacheTiming = "";
  unsigned long lastDraw = 0;

  while (digitalRead(BTN_PIN) == HIGH) {
    // อ่านระดับสัญญาณถี่ ๆ เพื่อวัดความกว้างพัลส์ (ISR ทำหน้าที่นับจำนวนอยู่แล้ว)
    bool level = dropLineActive();
    if (level != lastLevel) {
      uint32_t nowUs = micros();
      if (level) {
        if (lastRiseUs != 0) gapMs = (float)(uint32_t)(nowUs - lastRiseUs) / 1000.0f;
        lastRiseUs = nowUs;
        riseUs = nowUs;
      } else if (riseUs != 0) {
        widthMs = (float)(uint32_t)(nowUs - riseUs) / 1000.0f;
      }
      lastLevel = level;
    }
    ...
    if (millis() - lastDraw >= 80) {
      ...
      String lv = level ? "HIGH" : "LOW";
      if (lv != cacheLevel) {
        cacheLevel = lv;
        tft.fillRect(10, 96, 152, 26, THEME_NAVY);
        drawCenteredString(lv, 96, 3, level ? COLOR_YELLOW : ST77XX_LIGHTGREY, THEME_NAVY);
      }

      String cnt = String((uint32_t)(dropIsrCount - baseCount));
      if (cnt != cacheCount) {
        cacheCount = cnt;
        tft.fillRect(10, 160, 152, 26, THEME_NAVY);
        drawCenteredString(cnt, 160, 3, COLOR_WHITE, THEME_NAVY);
      }

      String w = (widthMs < 0) ? String("WIDTH  --") : ("WIDTH  " + String(widthMs, 1) + " ms");
      String g = (gapMs < 0)   ? String("GAP    --")
                               : ("GAP  " + String(gapMs / 1000.0f, 2) + " s  " +
                                  String(60000.0f / gapMs, 0) + " gtt/min");
      String timing = w + "|" + g;
      if (timing != cacheTiming) {
        cacheTiming = timing;
        tft.fillRect(10, 224, 152, 52, THEME_NAVY);
        drawCenteredString(w, 226, 1, COLOR_WHITE, THEME_NAVY);
        drawCenteredString(g, 244, 1, THEME_SKYBLUE, THEME_NAVY);
        if (dropIsrDropped > 0)
          drawCenteredString("QUEUE OVERFLOW " + String(dropIsrDropped), 262, 1, COLOR_RED, THEME_NAVY);
        else if (widthMs >= 0 && widthMs < 3.0f)
          drawCenteredString("PULSE TOO SHORT", 262, 1, COLOR_YELLOW, THEME_NAVY);
      }
    }
    delay(2);
  }
  ...
  // พัลส์ที่เกิดระหว่างทดสอบต้องไม่ถูกนับเป็นหยดจริง และต้องไม่ทำให้เตือนสายพับทันที
  clearDropQueue();
  if (hasFirstDropOccurred) {
    lastDropTimestamp = millis();
    lastDropMicros    = micros();
...
// ----------------------------------------------------------------------------
// ตรวจจับหยดน้ำจากเซ็นเซอร์เอาต์พุตดิจิทัล
//   ISR เก็บเฉพาะ "เวลาที่เกิดพัลส์" ลงคิว ทำงานสั้นที่สุดเท่าที่จะทำได้
//   ส่วนการคำนวณอัตราไหลทั้งหมดอยู่ใน serviceDropSensor() ที่ลูปหลักเรียก
// ----------------------------------------------------------------------------
void IRAM_ATTR onDropEdgeIsr() {
  static uint32_t lastAcceptedUs = 0;
  uint32_t nowUs = micros();
  if ((uint32_t)(nowUs - lastAcceptedUs) < (uint32_t)DROP_DEBOUNCE_MS * 1000UL) return;
  lastAcceptedUs = nowUs;
  dropIsrCount++;

  uint8_t next = (uint8_t)((dropQHead + 1) % DROP_QUEUE_SIZE);
  if (next == dropQTail) {          // คิวเต็ม (ลูปหลักค้างนานผิดปกติ) — นับไว้ให้เห็นในหน้าทดสอบ
    dropIsrDropped++;
    return;
  }
  dropQueueUs[dropQHead] = nowUs;
  dropQHead = next;
}

bool popDropEvent(uint32_t &outUs) {
  bool got = false;
  noInterrupts();
  if (dropQTail != dropQHead) {
    outUs = dropQueueUs[dropQTail];
    dropQTail = (uint8_t)((dropQTail + 1) % DROP_QUEUE_SIZE);
    got = true;
  }
  interrupts();
  return got;
...
// คำนวณผลของหยด 1 หยดที่เกิดขึ้นเมื่อเวลา eventUs (หน่วยไมโครวินาทีจาก micros())
void registerDrop(uint32_t eventUs) {
  unsigned long now = millis();
  ...
  if (hasFirstDropOccurred && !skipNextInterval) {
    // ลบกันแบบ uint32 จึงถูกต้องแม้ micros() จะวนรอบ (ทุก ~71 นาที)
    unsigned long dropDeltaMs = (unsigned long)((uint32_t)(eventUs - lastDropMicros) / 1000UL);
  ...
  skipNextInterval = false;
  lastDropMicros    = eventUs;
  lastDropTimestamp = now;
  currentGttMin = (currentFlowRate_ml_hr * (float)df) / 60.0f;
  totalVolumeMl = (float)totalDrops / (float)df;
}

void serviceDropSensor() {
  uint32_t eventUs;
  if (!isRunning) {          // ขณะหยุดชั่วคราว: ทิ้งพัลส์ที่เข้ามา ไม่นับเป็นหยด
    clearDropQueue();
    return;
  }
  while (popDropEvent(eventUs)) registerDrop(eventUs);
}

// เริ่มใช้งานขาเซ็นเซอร์ + ตรวจว่าสายไม่ได้ค้างอยู่ที่ระดับ "มีหยด"
void initDropSensor() {
#if DROP_ACTIVE_HIGH
  pinMode(DROP_DO_PIN, INPUT_PULLDOWN);   // ว่าง = LOW, มีหยด = HIGH
#else
  pinMode(DROP_DO_PIN, INPUT_PULLUP);     // ว่าง = HIGH, มีหยด = LOW
#endif
  delay(5);

  int activeCount = 0;
  for (int i = 0; i < 50; i++) {
    if (dropLineActive()) activeCount++;
    delay(2);
  }
  sensorLineStuck = (activeCount >= 48);  // ค้างเกือบตลอด 100 ms = ต่อสายผิดหรือปรับโวลุ่มเลยจุด

#if DROP_ACTIVE_HIGH
  attachInterrupt(digitalPinToInterrupt(DROP_DO_PIN), onDropEdgeIsr, RISING);
#else
  attachInterrupt(digitalPinToInterrupt(DROP_DO_PIN), onDropEdgeIsr, FALLING);
#endif
  clearDropQueue();
}
...
void updatePage3Dynamic() {
  ...
  bool doActive = dropLineActive();
  String sensorKey = String(doActive ? 1 : 0) + ":" + String(dropIsrCount) + ":" + String((int)currentGttMin);
  if (sensorKey != cacheP3b) {
    cacheP3b = sensorKey;
    textAt(padTo(doActive ? "DO HIGH" : "DO LOW", 10), 14, 118, 2,
           doActive ? COLOR_YELLOW : COLOR_WHITE, UI_CARD);
    textAt(padTo("PULSE " + String(dropIsrCount) + "   " + String(currentGttMin, 1) + " gtt/m", 22),
           14, 144, 1, UI_DIM, UI_CARD);
    textAt(padTo(sensorLineStuck ? "CHECK SENSOR WIRING"
                                 : ("DIGITAL  GPIO " + String(DROP_DO_PIN)), 22),
           14, 160, 1, sensorLineStuck ? COLOR_RED : UI_DIM, UI_CARD);
  }
